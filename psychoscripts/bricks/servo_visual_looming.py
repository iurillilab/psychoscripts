...
for i in range(ExpParams.n_looms):
    for trial_type in ["visual", "servo", "laser_only"]:
        laser_conditions = [True, False] if trial_type != "laser_only" else [True]
        for laser in laser_conditions:
            if trial_type == "laser_only":
                if i % 2 == 0:
                    servo_trials.append(
                        dict(trial_type=trial_type, laser=laser, position=0)
                    )
            else:
                for position in (
...
print(sum([trial["trial_type"] == "laser_only" for trial in servo_trials]))
